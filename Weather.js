...
function getWeatherData(){
   ...
   var oauth = "format=json" +
                  ...
                  "&oauth_version=1.0" + 
                  "&u=" + "c" 
      oauth = "GET&" + encodeURIComponent("https://weather-ydn-yql.media.yahoo.com/forecastrss") + "&" + encodeURIComponent(oauth)                  
      //hmac-sha1
      var signature = core_hmac_sha1(YAHOO_CONSUMER_SECRET, oauth)
      signature = binb2b64(signature) + "=";
      var headerAuthorization = "OAuth " +
         "oauth_consumer_key=\"" + YAHOO_CONSUMER_KEY + "\", " +
         "oauth_nonce=\"" + oauthNonce + "\", " +
         "oauth_timestamp=\"" + timestamp + "\", " +
         "oauth_signature_method=\"HMAC-SHA1\", " +
      ...
      IR.GetDevice("yahoo").SendEx({ 
         ...
         cbReceiveText:   
            function(text, code, headers){
               var yahooJson = JSON.Parse(text);
               IR.Log(yahooJson);
               IR.SetVariable("Server.Tags.currentTemp", yahooJson.current_observation.condition.temperature);
               IR.SetVariable("Server.Tags.currentState", yahooJson.current_observation.condition.code);
               IR.SetVariable("Server.Tags.sunrise", yahooJson.current_observation.astronomy.sunrise);
               IR.SetVariable("Server.Tags.sunset", yahooJson.current_observation.astronomy.sunset);
            }
      });
}
...
IR.SetInterval(60*30*1000, getWeatherData);